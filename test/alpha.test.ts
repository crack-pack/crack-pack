...
test('non-land counts match known Alpha rarity totals', () => {
  const nonLand = (r: Rarity) => alpha.sheets[r].cards.filter((c) => !c.isBasicLand).length;
...
test('common-sheet basic-land fill is 9P / 10I / 9S / 9M / 10F', () => {
  ...
  for (const c of alpha.sheets.common.cards) {
...
test('all basic-land variants are resolved to A or B (no anomalies remain)', () => {
  for (const r of RARITIES) {
    for (const c of alpha.sheets[r].cards) {
...
test('every drawn card actually lives on the sheet it was drawn from', () => {
  const onSheet: Record<Rarity, Set<string>> = {
    common: new Set(alpha.sheets.common.cards.map((c) => c.name)),
    uncommon: new Set(alpha.sheets.uncommon.cards.map((c) => c.name)),
    rare: new Set(alpha.sheets.rare.cards.map((c) => c.name)),
  };
  for (const pack of openPacks(alpha, 200, { seed: 7 })) {
...
test('pack 0 is the canonical first-ever Alpha pack (position 1 on all sheets)', () => {
  const pack = openPacks(alpha, 1, { startPack: 0 })[0];
...
test('a rare determines its pack — pack N is fully fixed by its index', () => {
  ...
  assert.deepEqual(openPacks(alpha, 1, { startPack: 7 }), openPacks(alpha, 1, { startPack: 7 }));
});

test('openings are reproducible for a fixed seed', () => {
  const a = openPacks(alpha, 10, { seed: 123 });
  const b = openPacks(alpha, 10, { seed: 123 });
  assert.deepEqual(a, b);
});

test('different seeds generally produce different packs', () => {
  const a = JSON.stringify(openPacks(alpha, 3, { seed: 1 }));
  const b = JSON.stringify(openPacks(alpha, 3, { seed: 2 }));
  assert.notEqual(a, b);
});
